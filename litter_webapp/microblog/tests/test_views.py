...
from microblog.models import Blogpost, Profile
...
class BlogpostListView(TestCase):
    ...
    def setUpTestData(cls):
        ...
        number_of_blogposts = 13
        for blogpost_num in range(number_of_blogposts):
           Blogpost.objects.create(blogger=profile,name='Test Blogpost %s' % blogpost_num,content='Test Blogpost %s Content' % blogpost_num)
    ...
    def test_view_uses_correct_template(self):
        resp = self.client.get(reverse('posts'))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, 'blogpost_list.html')
...
class ProfileListViewTest(TestCase):
    ...
    def test_view_url_accessible_by_name(self):
        resp = self.client.get(reverse('profiles'))
        self.assertEqual(resp.status_code, 200)

    def test_view_uses_correct_template(self):
        resp = self.client.get(reverse('profiles'))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, 'profiles/user.html')
...
class RegisterViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_user2 = User.objects.create_user(email='testuser2@mail.com', password='testing12345')
        test_user2.save()
        #permission = Permission.objects.get(name='Set book as returned')
        #test_user2.user_permissions.add(permission)
        #test_user2.save()

        test_profile = Profile.objects.create(username=test_user1)#(first_name='John', last_name='Smith')


    def test_redirect_if_not_logged_in(self):
        resp = self.client.get(reverse('login') )
        self.assertRedirects(resp, '/registration/login/' )

#    def test_redirect_if_logged_in_but_not_correct_permission(self):
#        login = self.client.login(username='testuser1', password='testing12345')
#        resp = self.client.get(reverse('login') )
...
#        self.assertEqual( resp.status_code,200)
    ...
    def test_uses_correct_template(self):
        login = self.client.login(email='testuser2@mail.com', password='testing12345')
        resp = self.client.get(reverse('login') )
        self.assertEqual( resp.status_code,200)
...
#        self.assertEqual(response_date, expected_initial_date )
    ...
    def test_redirects_to_home_view_on_success(self): #test_redirects_to_detail_view_on_success
        login = self.client.login(email='testuser2@mail.com', password='testing12345')
        resp = self.client.post(reverse('register'),{'email':'testuser2@mail.com'} )#{'first_name':'Alice','last_name':'Wonderland',}
        #Manually check redirect because we don't know what profile was created
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def setUp(self): # data that can be mutated later
        #Create two users
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def setUp(self):
        #Create a user
        test_user1 = User.objects.create_user(username='testuser1', password='12345') # usual user
        test_user1.save()

        test_user2 = User.objects.create_user(username='testuser2', password='12345')
        test_user2.save()
        permission = Permission.objects.get(name='Set book as returned') # test that only spec user has access to admin panel
        test_user2.user_permissions.add(permission)
        test_user2.save()
